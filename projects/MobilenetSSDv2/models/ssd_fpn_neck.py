...
from .mobilenet_v2_mmlab import InvertedResidual
...
class SSDFPNNeck(BaseModule):
    ...
    def __init__(self,
        ...
        self.smooth = nn.ModuleList(
            [InvertedResidual(self.target_channels, self.target_channels, 1, 0.5) for _ in range(len(out_channels)-1)]
        )
    ...
    def forward(self, inputs):
        ...
        p5 = self.smooth[4](p5 + F.interpolate(p6, scale_factor=2, mode='nearest'))
        p4 = self.smooth[3](p4 + F.interpolate(p5, scale_factor=2, mode='nearest'))
        p3 = self.smooth[2](p3 + F.interpolate(p4, scale_factor=2, mode='nearest'))
        p2 = self.smooth[1](p2 + F.interpolate(p3, scale_factor=2, mode='nearest'))
        p1 = self.smooth[0](p1 + F.interpolate(p2, scale_factor=2, mode='nearest'))
